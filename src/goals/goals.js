...
const Goals = {
  ...
  async addFunds(id) {
    ...
    if (window.Swal) {
      const { value } = await Swal.fire({ title: `Abonar a "${g.name}"`, input: 'number', inputLabelText: 'Monto', inputPlaceholder: '0', showCancelButton: true, confirmButtonColor: '#0370d4', background: UI.isDark()?'#0f172a':'#fff', color: UI.isDark()?'#e2e8f0':'#0f172a' });
